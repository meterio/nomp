...
module.exports = function(logger, portalConfig, poolConfigs) {
  ...
  this.handleApiRequest = async function(req, res, next) {
    switch (req.params.method) {
      ...
      case "blocks":
        var data = await portalStats.getBlockHistory();
        res.writeHead(200, { "Content-Type": "application/json" });
                  ...
                  for(var pool in portalStats.stats.pools) {
                    for(var w in portalStats.stats.pools[pool].workers){
                      if (w.startsWith(address)) {
                        ...
                        networkHashRate = portalStats.stats.pools[pool].poolStats.networkHashRate;
                      }
                    }
                  }
                  res.end(JSON.stringify({miner: address, totalHash: totalHash, totalShares: totalShares, networkHashRate: networkHashRate, immature: balances.totalImmature, balance: balances.totalHeld, paid: balances.totalPaid, workers: workers, history: history}));
                });
              });
            } else {
