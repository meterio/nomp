...
module.exports = function (logger) {
          ...
          poloApi.getTicker(function (err, data) {
            ...
            Object.keys(symbolToAlgorithmMap).forEach(function (symbol) {
              ...
              if (data.hasOwnProperty("BTC_" + symbol)) {
                if (!marketData.hasOwnProperty("BTC")) marketData["BTC"] = {};
              ...
              if (data.hasOwnProperty("LTC_" + symbol)) {
                if (!marketData.hasOwnProperty("LTC")) marketData["LTC"] = {};
  ...
  this.getMarketDepthFromPoloniex = function (
    ...
    poloApi.getOrderBook(symbolA, symbolB, function (err, data) {
      ...
      var marketData =
        profitStatus[symbolToAlgorithmMap[symbolB]][symbolB].exchangeInfo[
          "Poloniex"
        ];
            ...
            Object.keys(symbolToAlgorithmMap).forEach(function (symbol) {
              ...
              if (results && results.hasOwnProperty(symbol + "/BTC")) {
                ...
                if (btcData.buyorders != null) {
                  ...
                  var limit = new Number(
                    marketData["BTC"].bid * portalConfig.profitSwitch.depth
                  );
                  var depth = new Number(0);
                  var totalQty = new Number(0);
                  ...
                  if (totalQty > 0)
                    marketData["BTC"].weightedBid = new Number(
                      depth / totalQty
                    );
                }
              }
              ...
              if (results && results.hasOwnProperty(symbol + "/LTC")) {
                ...
                if (ltcData.buyorders != null) {
                  ...
                  var limit = new Number(
                    marketData["LTC"].bid * portalConfig.profitSwitch.depth
                  );
                  var depth = new Number(0);
                  var totalQty = new Number(0);
                  ...
                  if (totalQty > 0)
                    marketData["LTC"].weightedBid = new Number(
                      depth / totalQty
                    );
                }
              }
              ...
              response.data.forEach(function (market) {
                ...
                if (market.exchange == "BTC" && market.code == symbol) {
                  ...
                  marketData["BTC"].quoteVolume = new Number(
                    market["24hvol"] / market.last_price
                  );
      ...
      function (result) {
        if (result[0].error != null) {
          logger.error(
            logSystem,
            symbol,
            "Error while reading daemon info: " + JSON.stringify(result[0])
          );
          callback(null); // fail gracefully for each coin
          return;
        }
        ...
        var target = response.target
          ? new BigNumber("0x" + response.target)
          : util.bignumFromBitsHex(response.bits);
        coinStatus.difficulty = parseFloat(
          (diff1 / target.toNumber()).toFixed(9)
        );
        logger.debug(
          logSystem,
          symbol,
    ...
    Object.keys(profitStatus).forEach(function (algo) {
      Object.keys(profitStatus[algo]).forEach(function (symbol) {
        ...
        coinStatus.blocksPerMhPerHour =
          86400 /
          ((coinStatus.difficulty * Math.pow(2, 32)) / (1 * 1000 * 1000));
    ...
    Object.keys(profitStatus).forEach(function (algo) {
      ...
      Object.keys(profitStatus[algo]).forEach(function (symbol) {
        ...
        Object.keys(coinStatus.exchangeInfo).forEach(function (exchange) {
          ...
          if (
            ...
            var btcPerMhPerHour =
              exchangeData["BTC"].weightedBid * coinStatus.coinsPerMhPerHour;
            ...
            logger.debug(
              ...
              "BTC/" +
                ...
                exchange +
                " with " +
                coinStatus.btcPerMhPerHour.toFixed(8) +
                " BTC/day per Mh/s"
            );
          }
          if (
            exchangeData.hasOwnProperty("LTC") &&
            exchangeData["LTC"].hasOwnProperty("weightedBid")
          ) {
            var btcPerMhPerHour =
              exchangeData["LTC"].weightedBid *
              coinStatus.coinsPerMhPerHour *
              exchangeData["LTC"].ltcToBtc;
            if (btcPerMhPerHour > bestBtcPerMhPerHour) {
              bestBtcPerMhPerHour = btcPerMhPerHour;
              bestExchange = exchange;
              bestCoin = profitStatus[algo][symbol].name;
      ...
      logger.debug(
        ...
        "Best coin for " +
          ...
          bestExchange +
          " with " +
          bestBtcPerMhPerHour.toFixed(8) +
          " BTC/day per Mh/s"
      );

      var client = net
        .connect(portalConfig.cliPort, function () {
          client.write(
            JSON.stringify({
              command: "coinswitch",
              params: [bestCoin],
        ...
        .on("error", function (error) {
          if (error.code === "ECONNREFUSED")
            logger.error(
              logSystem,
              "CLI",
              "Could not connect to NOMP instance on port " +
                portalConfig.cliPort
            );
          else
            logger.error(
              logSystem,
              "CLI",
              "Socket error " + JSON.stringify(error)
            );
        });
    });
  };
    ...
    async.series(profitabilityTasks, function (err) {
      if (err) {
        logger.error(
          logSystem,
          "Check",
          "Error while checking profitability: " + err
        );
        return;
      }
      //
  ...
  setInterval(
    checkProfitability,
    portalConfig.profitSwitch.updateInterval * 1000
  );
};
